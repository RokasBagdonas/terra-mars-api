    ...
    <div class="tile is-5 is-vertical is-parent">
      ...
      <div class="table-container is-child box">
        <table class="table is-striped is-hoverable">
          <thead>
            <tr>
              <th v-for="name in GAME_SCHEMA" v-on:click="sortTable(name)">{{name}}</th>
            </tr>
          </thead>
          <tbody>
...
components: {GameDetails},
  ...
  methods: {
    fetchGames() {
      getGames(this.limit, this.currentOffset)
        .then((response) => {
    ...
    sortTable(name) {
      //TODO: api call for sorting by a name
      console.log("sort table event");
    },
    fetchGameScores(id){
        console.log("fetchGameScores");
        getGameScores(id)
        .then((response) =>{ this.pickedGame = response.data; console.log(response.data)})
        .catch((error) => console.log("error"));
    }
  },
  created: function () {
